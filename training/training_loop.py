...
def training_loop(
	...
	interface_kwargs = dict(img_resolution=dataset_obj.resolution, label_dim=dataset_obj.label_dim,
	                        img_channels=2 * dataset_obj.num_channels)
	...
	while True:
		...
		for round_idx in range(num_accumulation_rounds):
			with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
				...
				train_loss, val_loss, test_loss = loss_fn(net=ddp, images=images, labels=labels,
				                                          augment_pipe=augment_pipe,
				                                          corruption_matrix=corruption_matrix,
				                                          hat_corruption_matrix=hat_corruption_matrix)
				loss = val_loss
		...
		if (snapshot_ticks is not None) and (done or cur_tick % snapshot_ticks == 0):
			data = dict(ema=ema.state_dict(), loss_fn=loss_fn, augment_pipe=augment_pipe,
			...
			if dist.get_rank() == 0:
				with dnnlib.util.open_url(os.path.join(run_dir, f'network-snapshot-{cur_nimg // 1000:06d}.pkl'),
				                          verbose=(dist.get_rank() == 0), read_mode='wb') as f:
					pickle.dump(data, f)
			del data  # conserve memory
		...
		if (state_dump_ticks is not None) and (
			...
			torch.save(dict(net=net.state_dict(), optimizer_state=optimizer.state_dict()),
			           dnnlib.util.open_url(os.path.join(run_dir, f'training-state-{cur_nimg // 1000:06d}.pt'),
			                                verbose=(dist.get_rank() == 0), read_mode='wb'))

		# Update logs.
		training_stats.default_collector.update()
		if dist.get_rank() == 0:
			if stats_jsonl is None:
				stats_jsonl = dnnlib.util.open_url(os.path.join(run_dir, 'stats.jsonl'), read_mode='at',
