...
@click.option('--lr', help='Learning rate', metavar='FLOAT', type=click.FloatRange(min=0), default=10e-4,
              show_default=True)
...
@click.option('--max_grad_norm', help='Max norm for gradients.', metavar='FLOAT', type=click.FloatRange(min=0),
              default=None, show_default=True)
@click.option('--weight_decay', help='Value of weight decay. Set to 0. to disable.', metavar='FLOAT',
              type=click.FloatRange(min=0), default=0., show_default=True)
# Stochastic Sampling
@click.option('--S_churn', help='Amount of stochasticity', metavar='S_churn',
              type=click.FloatRange(min=0, max=float('inf')), default=10.0, show_default=True)
@click.option('--S_min', help='Saturation lower bound', metavar='S_min', type=click.FloatRange(min=0, max=1),
              default=0.01, show_default=True)
@click.option('--S_max', help='Saturation upper bound', metavar='S_max', type=click.FloatRange(min=0, max=1),
...
@click.option('--mask_full_rgb', help='Whether to mask all the RGB channels together', metavar='BOOL', default=False,
              show_default=True)
...
@click.option('--corruption_pattern', help='Corruption pattern', metavar='dust|box|downscale|fixed_box', default='dust',
...
# wandb
@click.option('--experiment_name', help='Name for the experiment to run', type=str, default=None, required=False,
              show_default=True)
def main(**kwargs):
	...
	c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=opts.data,
	                                   ...
	                                   delta_probability=opts.delta_probability, mask_full_rgb=opts.mask_full_rgb,
	...
	if opts.arch == 'ddpmpp':
		...
		c.network_kwargs.update(channel_mult_noise=1, resample_filter=[1, 1], model_channels=128,
		                        channel_mult=[2, 2, 2], gated=opts.gated)
