...
def open_url(url: str, cache_dir: str = None, num_attempts: int = 10, verbose: bool = True,
             return_filename: bool = False, cache: bool = True, read_mode: str = 'rb') -> Any:
...
def tensor_clipping(x, static=True, p=0.99):
	if static:
		...
		s_val = torch.tensor(np.percentile(torch.abs(x).detach().cpu().numpy(), p, axis=tuple(range(1, x.ndim))),
		                     device=x.device)
